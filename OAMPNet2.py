...
class OAMPNet2(object):
    ...
    def creat_graph(self):   
        ...
        for k in range(1, self.L+1):
            ...
            xhat.append(xhatk)
        print("Total number of trainable variables in OAMPNet2", self.get_n_vars())
    ...
    def layer(self, xhatt, rt):
        ...
        v2_t = tf.divide(tf.reduce_sum(tf.square(rt), axis=1, keepdims=True)-tf.cast(2*self.Nr,tf.float32)*self.noise_sigma2 /2., tf.expand_dims(tf.linalg.trace(HTH), axis=1))
        v2_t = tf.maximum(v2_t, 1e-9)
        v2_t = tf.expand_dims(v2_t, axis=1)
        lam_t = tf.expand_dims(self.noise_sigma2, axis=2)/2.
        inv_term_t = tf.linalg.inv(v2_t*HHT + lam_t*tf.eye(2*self.Nr, batch_shape=[self.batch_size])) # shape=(?,2*Nr,2*Nr )
        ...
        phi_t = tf.compat.v1.Variable(1.0, trainable=True)
        cesai_t = tf.compat.v1.Variable(1.0, trainable=True)
        xhatt = phi_t*(xhatt-cesai_t*z_t)
