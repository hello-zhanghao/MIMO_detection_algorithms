...
from DetNetSIC2 import DetNetSIC
...
if 'DetNetSIC' in params['simulation_algorithms']:
    ...
    DetNetSIC_nodes = DetNetSIC.creat_graph(ZF_initial=params['DetNetSIC_ZF_initial'])
...
if 'DetNetSIC' in params['simulation_algorithms']:
    loss_DetNetSIC = []
    print('===========DetNetSIC is Training, now_time is %s=========' % (datetime.datetime.now().strftime('%X')))
    train_st = time.time()
    for n in range(params['nRounds']):
        for epoch in range(params['maxEpoch']):
            x_Feed, H_Feed, y_Feed, noise_sigma2_Feed = gen_data.dataTrain(epoch, params['SNR_dB_train'])
            ...
            model_train(sess, DetNetSIC_nodes, data_Feed)
            if epoch % 1000 == 0:
                print('===========epoch%d,now_time is %s=========' % (epoch + n*params['maxEpoch'],
                                                                      datetime.datetime.now().strftime('%X')))
                ser_DetNetSIC, loss = model_loss(sess, DetNetSIC_nodes, data_Feed)
                print('ser_DetNetSIC', ser_DetNetSIC, 'loss=', loss, '\n')
                loss_DetNetSIC.append(loss)

    loss_all['DetNetSIC'] = loss_DetNetSIC
    train_ed = time.time()
    print("DetNetSIC Train time is: " + str(train_ed - train_st))
...
if params['isTest']:
    ...
    if 'DetNetSIC' in params['simulation_algorithms']:
        ...
        ser_DetNetSICs = model_eval(sess, params, DetNetSIC_nodes, gen_data, params['test_iterations'])
        results['ser_DetNetSICs'] = ser_DetNetSICs
        print(ser_DetNetSICs)
