...
DetNet_nodes = DetNet(params).creat_graph(ZF_initial=params['DetNet_ZF_initial'])
...
for n in range(params['nRounds']):
    for epoch in range(params['maxEpoch']):
        ...
        if epoch % 100 == 0:
            ...
            ser, loss = model_loss(sess, DetNet_nodes, data_Feed)
...
sers = model_eval(sess, params, DetNet_nodes, gen_data, params['test_iterations'])
results['ser_DetNets'] = sers
print(sers)
